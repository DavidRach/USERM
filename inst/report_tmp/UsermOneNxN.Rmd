...
for (i in 1:N) {
  for (j in 1:N) {
    ...
    if (i > j) {
      ...
      mu_x = intensity_col[fluor_selected[i],1]
      mu_y = intensity_col[fluor_selected[j],1]
      sigma_x = sqrt_col[fluor_selected[i],1]
      sigma_y = sqrt_col[fluor_selected[j],1]
  
      x <- create_seq(min = Scale_df[fluor_selected[i],"min"],
                      max = Scale_df[fluor_selected[i],"max"], len=100,
                      scale = Scale_df[fluor_selected[i],"scale"], cofactor = Scale_df[fluor_selected[i],"cofactor"])
      ...
      p = PredIsobandPlot(grid = grid,
                          x_scale = Scale_df[fluor_selected[i],"scale"],
                          y_scale = Scale_df[fluor_selected[j],"scale"],
                          x_cofactor = Scale_df[fluor_selected[i],"cofactor"],
                          y_cofactor = Scale_df[fluor_selected[j],"cofactor"],
                          x_label = fluor_selected[i],
                          y_label = fluor_selected[j],
                          x_min = Scale_df[fluor_selected[i],"min"],
                          x_max = Scale_df[fluor_selected[i],"max"],

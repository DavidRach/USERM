...
Intercept matrix represents the spread of negative population. Since the intercept matrix may differ between estimations of fluorescences, we use a "weighted intercept matrix" (median) as a robust estimation for the USERM spread prediction. If the raw data is of high quality with enough sample size, the intercept matrix should be stable and consistant.
...
final_col = matrix(apply(weighted_intercept_matrix,1,median),ncol = 1)
sqrt_col = sqrt(abs(final_col))
final_matrix = cbind(weighted_intercept_matrix, final_col)
